...
#define DEVELOPMENT
#include "EncridWavefrontLoader.h"
...
int TDE_WavefrontLoader::Load(TDE_Device* dev, const char* filename)
{
    ...
    for(int i=0;i<coord.size();i++) //and then go through all line and decide what kind of line it is
    {
        ...
        else if((*coord[i])[0]=='v' && (*coord[i])[1]==' ')     //if a vertex
        {
            ...
            vertex.push_back(new TDE_Coordinate(tmpx,tmpy,tmpz));       //and put it in the vertex vector
        }
		else if ((*coord[i])[0]=='v' && (*coord[i])[1]=='n')    //if it's a normal vector
        {
            float tmpx,tmpy,tmpz;
            sscanf(coord[i]->c_str(),"vn %f %f %f",&tmpx,&tmpy,&tmpz);
            normals.push_back(new TDE_Coordinate(tmpx,tmpy,tmpz));      //basically do the same
            isnormals=true;
        }
		else if ((*coord[i])[0]=='f')   //if it's a face
        {
            ...
            if(count(coord[i]->begin(),coord[i]->end(),' ')==4)     //if this is a quad
            {
                if(coord[i]->find("//")!=std::string::npos)     //if it's contain a normal vector, but not contain texture coorinate
                {
                    ...
                    faces.push_back(new TDE_Face(b,a,c,d,e,0,0,0,0,curmat));    //and put to the faces, we don't care about the texture coorinate in this case
                }
				else if(coord[i]->find("/")!=std::string::npos)        //if we have texture coorinate and normal vectors
                {
                    ...
                    faces.push_back(new TDE_Face(b,a,c,d,e,t[0],t[1],t[2],t[3],curmat));
                }
				else
				{
                    //else we don't have normal vectors nor texture coorinate
                    sscanf(coord[i]->c_str(),"f %d %d %d %d",&a,&b,&c,&d);
                    faces.push_back(new TDE_Face(-1,a,b,c,d,0,0,0,0,curmat));          
                }
            }
			else
			{  //if it's a triangle
                ...
                if(coord[i]->find("//")!=std::string::npos)
                {
                    ...
                    faces.push_back(new TDE_Face(b,a,c,d,0,0,0,curmat));
                }
				else if (coord[i]->find("/")!=std::string::npos)
                {
                    int t[3];
                    sscanf(coord[i]->c_str(),"f %d/%d/%d %d/%d/%d %d/%d/%d",&a,&t[0],&b,&c,&t[1],&b,&d,&t[2],&b);
                    faces.push_back(new TDE_Face(b,a,c,d,t[0],t[1],t[2],curmat));
                }
				else
				{
                    sscanf(coord[i]->c_str(),"f %d %d %d",&a,&b,&c);
                    faces.push_back(new TDE_Face(-1,a,b,c,0,0,0,curmat));                                      
                }
            }
        }
		...
		else if((*coord[i])[0]=='m' && (*coord[i])[1]=='t' && (*coord[i])[2]=='l' && (*coord[i])[3]=='l')      //material library, a file, which contain                                                                                                                                                                                                                                                                                                                                                                                                                //all of the materials
        {
            char filen[500];
			char mtllib[500];
			////
			...
			{
				dir = fn.substr(0, last_slash_idx);
				strcpy(filen, dir.c_str());
			    //directory = fn.substr(0, last_slash_idx);
			}
			////
            sscanf(coord[i]->c_str(),"mtllib %s",mtllib);    //read the filename
			strcat(filen, "\\");
			strcat(filen, mtllib);
			//
            ...
            if(!mtlin.is_open())    //if not opened error message, clean all memory, return with -1
            {
                ...
                MessageBox(NULL, L"cannot open the material file", L"Errpr", MB_OK);
                Clean();
                return -1;
            }
            ...
            for(int i=0;i<tmp.size();i++) //go through all lines of the mtllib file
            {
                ...
                if(tmp[i][0]=='n' && tmp[i][1]=='e' && tmp[i][2]=='w')  //new material
                {
                    if(ismat)       //if we have a material
                    {
                        ...
                        {
                            materials.push_back(new TDE_Material(name,alpha,ns,ni,dif,amb,spec,illum,texture)); //push back
                            strcpy(filename,"\0");
                        }
						else
						{
                            materials.push_back(new TDE_Material(name,alpha,ns,ni,dif,amb,spec,illum,-1));              //push back, but use -1 to texture             
                        }
                    }
				...
				else if(tmp[i][0]=='m' && tmp[i][1]=='a')      //and the texture
                {
					...
					char fnDir[500];
					strcpy(fnDir, dir.c_str());
					strcat(fnDir, "\\");
					strcat(fnDir, filename);
					//
                    texture=LoadTexture(dev, fnDir);  //read the filename, and use the loadTexture function to load it, and get the id.
                    ismat=true;
                }
            }
            if(ismat)       //there is no newmat after the last newmat, so we have to put the last material 'manually'
            {
                ...
                {
                    materials.push_back(new TDE_Material(name,alpha,ns,ni,dif,amb,spec,illum,texture));
                }
				else
				{
                    materials.push_back(new TDE_Material(name,alpha,ns,ni,dif,amb,spec,illum,-1));                             
                }
            }
        }
        else if((*coord[i])[0]=='v' && (*coord[i])[1]=='t')    //back to the obj file, texture coorinate
        {
            ...
            texturecoordinate.push_back(new TDE_TexCoordinate(u,1-v));       //I push back 1-v instead of normal v, because obj file use the upper left corner as 0,0 coorinate
...
void TDE_WavefrontLoader::Clean()
{
...
TDE_WavefrontLoader::~TDE_WavefrontLoader()
{
...
unsigned int TDE_WavefrontLoader::LoadTexture(TDE_Device* dev, char* filename)
{
    //nothing new in here
    //unsigned int num;
    //glGenTextures(1,&num);
    //SDL_Surface* img=SDL_LoadBMP(filename);
	TDE_Image* tex = new TDE_Image();
	tex->LoadImageFromFile(dev, filename);
